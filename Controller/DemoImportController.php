...
class DemoImportController extends BaseController
{
    public function import()
    {
        ...
        $tasks = $this->db->table('tasks')->eq('project_id', $project_id)->findAll();
        ...
        $this->db->table('tasks')->insert([
            ...
            'date_modification' => $base_time,
            'date_moved' => $base_time,
        ...
        $t1 = $this->db->getLastId();
        
        $this->db->table('subtasks')->insert(['task_id' => $t1, 'title' => 'Apache .htaccess ve ModSecurity rewrite kurallarının incelenmesi', 'status' => 2, 'time_estimated'=>12, 'time_spent'=>12, 'user_id' => $user_id]);
        $this->db->table('subtasks')->insert(['task_id' => $t1, 'title' => 'Forgejo yerel entegrasyonu ve pnpm monorepo yapısının kurulması', 'status' => 2, 'time_estimated'=>4, 'time_spent'=>4, 'user_id' => $user_id]);
        ...
        $this->db->table('tasks')->insert([
            ...
            'date_modification' => $base_time + (2 * 86400),
            'date_moved' => $base_time + (2 * 86400),
        ...
        $t2 = $this->db->getLastId();
        
        $this->db->table('subtasks')->insert(['task_id' => $t2, 'title' => 'ExecutiveDashboardController.php içinde score() ve performance() metotlarının yazılması', 'status' => 2, 'time_estimated'=>3, 'time_spent'=>3, 'user_id' => $user_id]);
        $this->db->table('subtasks')->insert(['task_id' => $t2, 'title' => 'Görev karmaşıklığı (score) ve öncelik çarpanlarının (priority) ceza formüllerine entegre edilmesi', 'status' => 2, 'time_estimated'=>3, 'time_spent'=>3, 'user_id' => $user_id]);
        ...
        $this->db->table('tasks')->insert([
            ...
            'date_modification' => $base_time + (4 * 86400),
            'date_moved' => $base_time + (4 * 86400),
        ...
        $t3 = $this->db->getLastId();
        
        $this->db->table('subtasks')->insert(['task_id' => $t3, 'title' => 'budget_lines (Tahsis edilen bütçe havuzu) ile zaman takibi maliyetlerinin ayrıştırılması', 'status' => 2, 'time_spent'=>2, 'user_id' => $user_id]);
        $this->db->table('subtasks')->insert(['task_id' => $t3, 'title' => '/project/{id}/budget temiz URL (clean URL) yönlendirmelerinin kusursuz hale getirilmesi', 'status' => 2, 'time_spent'=>2, 'user_id' => $user_id]);
        ...
        $this->db->table('tasks')->insert([
            ...
            'date_modification' => $base_time + (6 * 86400),
            'date_moved' => $base_time + (6 * 86400),
        ...
        $t4 = $this->db->getLastId();
        
        $this->db->table('subtasks')->insert(['task_id' => $t4, 'title' => 'Geciken görevler ve kritik blokajlar için JS filtreleme butonlarının eklenmesi', 'status' => 2, 'time_spent'=>1.5, 'user_id' => $user_id]);
        $this->db->table('subtasks')->insert(['task_id' => $t4, 'title' => 'health.php şablonunun modern glassmorphism/minimalist estetiğe uyarlanması', 'status' => 2, 'time_spent'=>1.5, 'user_id' => $user_id]);
        ...
        $this->db->table('task_has_links')->insert(['task_id' => $t1, 'opposite_task_id' => $t2, 'link_id' => 2]); 
        $this->db->table('task_has_links')->insert(['task_id' => $t2, 'opposite_task_id' => $t3, 'link_id' => 2]); 
        $this->db->table('task_has_links')->insert(['task_id' => $t3, 'opposite_task_id' => $t4, 'link_id' => 2]); 
        ...
        echo "<p>Tüm eklenti kancaları (hooks) baypas edildi ve eksik SQL değerleri (swimlane_id=0) elle dolduruldu.</p>";
